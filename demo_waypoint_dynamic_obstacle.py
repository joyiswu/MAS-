...
class arm_base_printing:
    ...
    def print_future_visualize(self, way_points, index, step = 3, length = 3):

        # print index, len(way_points)
        if len(way_points) > step + length + 1:
            way_points = [way_points[0]] + way_points
            if index == 0: differential = map(self.future_visualize, 
                                              way_points[step:step+length], \
                                              way_points[step+1:step+length+1])
            elif len(way_points) > index + step + 2:
                self.future_visualize(way_points[index + step + 1], way_points[index + step + 2])
